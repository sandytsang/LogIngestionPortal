...
export function generateDeployReadme(
  ...
  if (config.action === 'updateColumns') {
    ...
    const updCloudShellFlags = [...updFlags, '-SchemaPath /home/<your-user>/columns.json'];
    const updCloudShellCommand = [
      '  pwsh ./scripts/deploy.ps1 `',
      ...updCloudShellFlags.map(
        (f, i) => `    ${f}${i < updCloudShellFlags.length - 1 ? ' `' : ''}`,
      ),
    ];
    return [
      ...
      '',
      'If you cannot install tools locally, run the same schema-only update in Azure',
      'Cloud Shell. Use the uploaded file path shown by Cloud Shell for -SchemaPath',
      '(commonly /home/<your-user>/columns.json).',
      '',
      '  1) Open https://shell.azure.com and sign in.',
      '  2) Upload your generated columns.json file (Cloud Shell usually stores',
      '     uploads under /home/<your-user>/).',
      '  3) Clone this repo (or upload/unzip the generated LogIngestionAPI folder)',
      '     and run the Cloud Shell command from this README.txt:',
      '',
      '       cd ~',
      ...
      '',
      '  4) Keep -SchemaPath /home/<your-user>/columns.json and',
      '     -Subscription <subscription-name-or-id>.',
      '  5) Run:',
      ...updCloudShellCommand,
      '',
      'GitHub workflow option',
      '----------------------',
      'You can also run the "Update data columns (schema-only)" GitHub workflow in',
      'your fork of https://github.com/sandytsang/LogIngestionPortal instead of',
      'running the script yourself — it applies the same -SchemaOnly update.',
      '',
  ...
  return [
    ...
    '',
    'The GitHub Action checks this permission and will warn if it cannot verify or',
    'assign Device.Read.All. Assigning a Graph app role requires a Graph admin',
    '(Privileged Role Administrator / Global Administrator) — which in most',
    'companies is a DIFFERENT person than whoever runs the deploy. If the deploy',
    'prints a warning, have that admin run the helper with these exact values:',
    '',
